 ...
 */
export function ratelimitRemote(
	issues: Issues,
	category: string,
	getClientAddress: () => string,
	timeWindow: number,
	...
	const isLimited = rl(category, getClientAddress, timeWindow, maxRequests)
	if (isLimited) return issues(msg)
}
...
export default (
	...
	const isLimited = rl(category, getClientAddress, timeWindow, maxRequests)
	if (isLimited) return fail(429, { msg })
}
