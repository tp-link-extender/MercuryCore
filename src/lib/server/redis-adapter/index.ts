...
const adapter =
	(redisClient: { session: RedisClientType<any, any, any>; userSessions: RedisClientType<any, any, any> }): AdapterFunction<SessionAdapter> =>
	() => {
		...
		return {
			...
			getSessionsByUserId: async userId => {
				...
				const sessions = sessionData.filter((val): val is string => val !== null).map(val => JSON.parse(val) as SessionSchema)
				return sessions
			},
			...
			deleteSession: async (...sessionIds) => {
				...
				const sessions = targetSessionData.filter((val): val is string => val !== null).map(val => JSON.parse(val) as SessionSchema)
				await Promise.all([...sessionIds.map(id => sessionRedis.del(id)), ...sessions.map(session => userSessionsRedis.lRem(session.user_id, 1, session.id))])
			},
			deleteSessionsByUserId: async userId => {
				const sessionIds = await userSessionsRedis.lRange(userId, 0, -1)
				await Promise.all([...sessionIds.map(id => sessionRedis.del(id)), userSessionsRedis.del(userId)])
			},
		}
	}
