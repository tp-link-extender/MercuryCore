<script lang="ts">
	...
	const currentColours: any = {
		"Head":brickToHex($user?.bodyColours["Head"]),
		"Torso":brickToHex($user?.bodyColours["Torso"]),
		"RightArm":brickToHex($user?.bodyColours["RightArm"]),
		"LeftArm":brickToHex($user?.bodyColours["LeftArm"]),
		"RightLeg":brickToHex($user?.bodyColours["RightLeg"]),
		"LeftLeg":brickToHex($user?.bodyColours["LeftLeg"])
	}

	function brickToHex(brickColor: number) {
		const hex: any = {
			1: "F2F3F3",
			2: "A1A5A2",
			3: "F9E999",
	...
	const pickColor = (color: string) => async () => {
		console.log(color)
		...
		const response = await fetch(`avatar?/paint`, { method: "POST", body: formdata })
		const colorRes = deserialize(await response.text())

		if(colorRes.status == 200) {
			currentColours[bodyPart] = color
		}
						...
						<div class="mx-auto" style="height:240px;width:194px;text-align:center;">
							<div style="position: relative; margin: 11px 4px; height: 1%;">
								<div style="position: absolute; left: 72px; top: 0px; cursor: pointer">
									<div class="bodyPart" on:click={openColorPicker("Head")} on:keydown={openColorPicker("Head")} style="background-color:#{currentColours["Head"]};height:44px;width:44px;" />
								</div>
								<div style="position: absolute; left: 0px; top: 52px; cursor: pointer">
									<div class="bodyPart" on:click={openColorPicker("RightArm")} on:keydown={openColorPicker("RightArm")} style="background-color:#{currentColours["RightArm"]};height:88px;width:40px;" />
								</div>
								<div style="position: absolute; left: 48px; top: 52px; cursor: pointer">
									<div class="bodyPart" on:click={openColorPicker("Torso")} on:keydown={openColorPicker("Torso")} style="background-color:#{currentColours["Torso"]};height:88px;width:88px;" />
								</div>
								<div style="position: absolute; left: 144px; top: 52px; cursor: pointer">
									<div class="bodyPart" on:click={openColorPicker("LeftArm")} on:keydown={openColorPicker("LeftArm")} style="background-color:#{currentColours["LeftArm"]};height:88px;width:40px;" />
								</div>
								<div style="position: absolute; left: 48px; top: 146px; cursor: pointer">
									<div
										...
										style="background-color:#{currentColours["LeftLeg"]};height:88px;width:40px;"
									/>
								</div>
								<div style="position: absolute; left: 96px; top: 146px; cursor: pointer">
									<div class="bodyPart" on:click={openColorPicker("RightLeg")} on:keydown={openColorPicker("RightLeg")} style="background-color:#{currentColours["RightLeg"]};height:88px;width:40px;" />
								</div>
							</div>
						</div>
					</div>
		...
		<div class="modal-body d-flex flex-column p-1">
			<div class="colourPicker text-left mx-auto" style="max-width:351px">
				{#each bodyColours as colours}
					<div class="colour m-1" on:click={pickColor(colours)} on:keydown={pickColor("ya")} style="display:inline-block;background-color:#{colours};height:40px;width:40px;" />
				{/each}
			</div>
		</div>
	</Modal>
