...
<div class="container mt-4">
	...
	<ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
		<li class="nav-item" role="presentation">
			...
			> 		</li>
		<li class="nav-item" role="presentation">
			<button class="nav-link" id="pills-network-tab" data-bs-toggle="pill" data-bs-target="#pills-network" type="button" role="tab" aria-controls="pills-network" aria-selected="false" tabindex={-1}
				><i class="fa-solid fa-network-wired"></i> Network</button
			>
		</li>
		<li class="nav-item" role="presentation">
			<button
				...
				aria-selected="false"
				tabindex={-1}><i class="fa-solid fa-eye-low-vision"></i> Privacy</button
			>
		</li>
	</ul>
	<div class="tab-content light-text mt-4" id="pills-tabContent">
		<div class="tab-pane fade active show" id="pills-view" role="tabpanel" aria-labelledby="pills-view-tab" tabindex={0}>
			...
			<form class="col-lg-8" method="POST" action="?/view" use:enhance>
				<fieldset>
					<div class="row mb-2">
						<label for="title" class="col-md-3 col-form-label text-md-right">Title</label>
						<div class="col-md-9">
							<input id="title" required name="title" value={data.name} class="form-control {form?.area == 'title' ? 'is-invalid' : 'valid'}"/>
							{#if form?.area == "title"}
					...
					<div class="row">
						...
						<div class="container">
							<textarea required class="form-control light-text mb-1 {form?.area == 'desc' ? 'is-invalid' : 'valid'}" placeholder="Maximum 1000 characters" id="desc" name="desc" rows={3} value={data.description} />
						</div>
					</div>
				</fieldset>
				...
				{#if form?.viewsuccess}
					...
					<noscript class="text-warning mt-3">Javascript is disabled, you may have to reload to apply changes.</noscript>
				{/if}
			</form>
		</div>
		<div class="tab-pane fade" id="pills-network" role="tabpanel" aria-labelledby="pills-network-tab" tabindex={0}>
			...
			<form class="col-lg-8" method="POST" action="?/network" use:enhance>
				<fieldset>
					<div class="row mb-2">
						<label for="address" class="col-md-3 col-form-label text-md-right">Address</label>
						<div class="col-md-9">
							<input id="address" required name="address" value={data.serverIP} class="form-control {form?.area == 'address' ? 'is-invalid' : 'valid'}"/>
					...
					<div class="row mb-2">
						...
						<div class="col-md-9">
							<input id="port" required name="port" value={data.serverPort} class="form-control {form?.area == 'port' ? 'is-invalid' : 'valid'}"/>
							{#if form?.area == "port"}
								<small class="col-12 mb-3 text-danger">{form?.msg}</small>
							{/if}
						</div>
					...
					<div class="row mb-2">
						...
						<div class="col-md-9">
							<div class="input-group">
								<input id="ticket" required name="ticket" value={data.serverTicket} class="form-control valid" disabled/>
								{#if form?.area == "ticket"}
									<small class="col-12 mb-3 text-danger">{form?.msg}</small>
								{/if}
		...
		<div class="tab-pane fade" id="pills-privacy" role="tabpanel" aria-labelledby="pills-privacy-tab" tabindex={0}>
			...
			<form class="col-lg-8" method="POST" action="?/privacy" use:enhance>
				<fieldset>
					<div class="row mb-2">
					...
					<div class="row mb-2">
						...
						<div class="col-md-9">
							<div class="input-group">
								<input id="privateLink" name="privateLink" value={`https://banland.xyz/place/${data.id}/${data.name}?privateServer=${data.privateTicket}`} class="form-control valid" disabled/>
								{#if form?.area == "privacy"}
									<small class="col-12 mb-3 text-danger">{form?.msg}</small>
								{/if}
								<button on:click={copyPrivateLink} class="btn btn-info" type="button" id="button-addon2"><i class="fa-solid fa-paste"></i></button>
