<script lang="ts">
    function openPictures() {
        window.external.OpenPicFolder() // roblox
    }
    function noBother() {
        window.external.PostImage(false, 0); // roblox 2
        window.close();
    }
</script>
<div class="app container py-4 light-text">
	...
	<ul>
		<li>
			Open <button on:click={openPictures} class="btn btn-sm btn-primary"><i class="far fa-folder-open"/> My Pictures</button> to check it out!
		</li>
		<li>Paste it to your favorite painting software</li>
	</ul>
	<hr class="divider-bottom" />
    <button class="btn btn-sm btn-danger"><i class="fas fa-ban"></i> Not interested</button>
