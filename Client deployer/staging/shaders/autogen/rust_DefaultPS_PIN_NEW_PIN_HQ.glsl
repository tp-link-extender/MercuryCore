...
void main ()
{
  ...
  vec4 diffuse_6;
  vec4 tmpvar_7;
  tmpvar_7 = mix (texture2D (DiffuseMap, (xlv_TEXCOORD0.xy * 0.5)), texture2D (DiffuseMap, xlv_TEXCOORD0.xy), vec4(clamp (((tmpvar_3 * 4.0) - 3.0), 0.0, 1.0)));
  ...
  albedo_4 = (((mix (vec3(1.0, 1.0, 1.0), xlv_COLOR0.xyz, vec3(mix (1.0, tmpvar_7.w, tmpvar_3))) * diffuse_6.xyz) * (1.0 + (normal_5.x * 0.5))) * (texture2D (StudsMap, xlv_TEXCOORD1.xy).xyz * 2.0));
  vec2 tmpvar_10;
  tmpvar_10 = mix (vec2(0.35, 103.0), ((texture2D (SpecularMap, xlv_TEXCOORD0.xy).xy * vec2(1.0, 256.0)) + vec2(0.0, 0.01)), vec2(tmpvar_3));
  vec4 tmpvar_11;
