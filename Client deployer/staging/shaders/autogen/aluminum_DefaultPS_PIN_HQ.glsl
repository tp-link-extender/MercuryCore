...
void main ()
{
  ...
  vec3 tNorm_4;
  vec2 tmpvar_5;
  tmpvar_5 = (xlv_TEXCOORD0.xy * 0.2);
  vec2 tmpvar_6;
  tmpvar_6 = (tmpvar_5 * 0.4);
  vec3 tmpvar_7;
  tmpvar_7 = (mix (mix (texture2D (NormalMap, tmpvar_5).xyz, texture2D (NormalMap, tmpvar_6).xyz, vec3(0.5, 0.5, 0.5)), texture2D (NormalMap, (tmpvar_6 * 0.1)).xyz, vec3(0.3, 0.3, 0.3)) - 0.5);
  tNorm_4.z = tmpvar_7.z;
  tNorm_4.xy = (tmpvar_7.xy * tmpvar_3);
  vec4 tmpvar_8;
  tmpvar_8.w = 0.0;
  tmpvar_8.xyz = (xlv_COLOR0.xyz * (0.4 + dot (tmpvar_7, vec3(0.6, 0.6, 0.6))));
  vec4 tmpvar_9;
  tmpvar_9 = texture2D (StudsMap, xlv_TEXCOORD1.xy);
  vec4 tmpvar_10;
  tmpvar_10.xyz = mix (mix (xlv_COLOR0, tmpvar_8, vec4(tmpvar_3)).xyz, tmpvar_9.xyz, tmpvar_9.www);
  tmpvar_10.w = xlv_COLOR0.w;
  albedo_2.w = tmpvar_10.w;
  vec3 tmpvar_11;
  tmpvar_11 = normalize((((tNorm_4.x * xlv_TEXCOORD6) + (tNorm_4.y * ((xlv_TEXCOORD4.yzx * xlv_TEXCOORD6.zxy) - (xlv_TEXCOORD4.zxy * xlv_TEXCOORD6.yzx)))) + (tmpvar_7.z * xlv_TEXCOORD4.xyz)));
  float tmpvar_12;
  ...
  albedo_2.xyz = mix (tmpvar_10.xyz, textureCube (EnvironmentMap, (i_17 - (2.0 * (dot (tmpvar_11, i_17) * tmpvar_11)))).xyz, vec3((0.35 * tmpvar_3)));
  oColor0_1.xyz = ((((AmbientColor + (((clamp (tmpvar_12, 0.0, 1.0) * Lamp0Color) + (max (-(tmpvar_12), 0.0) * Lamp1Color)) * tmpvar_16.w)) + tmpvar_16.xyz) * albedo_2.xyz) + (Lamp0Color * (((float((tmpvar_12 >= 0.0)) * 0.9) * tmpvar_16.w) * pow (clamp (dot (tmpvar_11, normalize((-(Lamp0Dir) + normalize(xlv_TEXCOORD3.xyz)))), 0.0, 1.0), 25.0))));
