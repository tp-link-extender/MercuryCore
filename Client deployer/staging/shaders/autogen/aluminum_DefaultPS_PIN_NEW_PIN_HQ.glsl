...
void main ()
{
  ...
  vec3 normal_5;
  vec2 tmpvar_6;
  tmpvar_6 = ((mix (texture2D (NormalMap, (xlv_TEXCOORD0.xy * 0.25)), texture2D (NormalMap, xlv_TEXCOORD0.xy), vec4(clamp (((tmpvar_3 * 4.0) - 3.0), 0.0, 1.0))).wy * 2.0) - 1.0);
  vec3 tmpvar_7;
  ...
  vec4 tmpvar_8;
  tmpvar_8 = texture2D (SpecularMap, xlv_TEXCOORD0.xy);
  vec2 tmpvar_9;
  tmpvar_9 = mix (vec2(0.94, 240.0), ((tmpvar_8.xy * vec2(1.0, 256.0)) + vec2(0.0, 0.01)), vec2(tmpvar_3));
  vec4 tmpvar_10;
  ...
  albedo_2.xyz = mix (albedo_4, textureCube (EnvironmentMap, (i_17 - (2.0 * (dot (tmpvar_11, i_17) * tmpvar_11)))).xyz, vec3(((tmpvar_8.y * tmpvar_3) * 0.6)));
